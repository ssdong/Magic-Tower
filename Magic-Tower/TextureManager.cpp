...
void textureManager::drawMessage(std::string messageType, 
	                     ...
	                     SDL_Texture* texture_Level;
	                     SDL_Rect destRect;
						 std::unordered_map<Position, SDLGameObject*, PositionHash>* elements = &(floor->elements);
							 ...
							 while(true) {
								...
								unsigned int currentTime = SDL_GetTicks();
						  	    if(currentTime > lastTime + 500) {
									   ...
									   // Need to clear first, otherwise images will overlap on the screen
									   ...
									   string total;
									   if(messageType == "key"){
									       if((*elements)[position]->getColor() == "yellow") { 
											       color = DARKYELLOW ;
		                                           total = "A yellow key gained.";
									       }
									       else if((*elements)[position]->getColor() == "red") {
											       color = RED;
										   ...
										   if((*elements)[position]->getColor() == "red") {
											       color = RED;
										           total = "50 HP recover.";
									       } 
										   else if((*elements)[position]->getColor() == "blue") {
											       color = BLUE;
										           total = "100 HP recover.";
									       } 
										   else if((*elements)[position]->getColor() == "green") {
											       color = GREEN;
										           total = "150 HP recover.";
									       } 
										   else if((*elements)[position]->getColor() == "yellow") {
											       color = YELLOW;
							 ...
							 (*elements).erase(position);
							 	 ...
							 	 while(true) {
										...
										if(currentTime > lastTime + 1000) {
	                                        ...
	                                        player->draw();
			                                drawStatus("hero","item",player,renderer,font);
											drawFight("hero","enemy",player,(*elements)[position],renderer,font,true,0,0);
	                                        SDL_RenderPresent(renderer);
										    SDL_Event Event;
										    while(SDL_PollEvent(&Event)){ }
										}
	                               ...
	                               player->draw();
                                   drawStatus("hero","item",player,renderer,font);
						           drawFight("hero","enemy",player,(*elements)[position],renderer,font,false,playerHP,enemyHP);
	                               SDL_RenderPresent(renderer);
							       SDL_Event Event;
							       while(SDL_PollEvent(&Event)){ }
						 }
